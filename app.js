...
/* ---------------------------------------------------------
   PROFIL PRINCIPAL (étape 10) — Type Événement / Service
   --------------------------------------------------------- */
...
WorkerTSA.validateMainProfile = async function () {
  const errorEl = document.getElementById('org-10-error');
  errorEl.classList.remove('visible');
...
WorkerTSA.openTicketPurchase = async function (eventId) {
  let event = (WorkerTSA.state.participantEvents || []).find(function (e) { return e.id === eventId; });
  if (!event) {
    try {
      const doc = await db.collection('events').doc(eventId).get();
      if (doc.exists) event = Object.assign({ id: doc.id }, doc.data());
    } catch (error) {
      console.error('Impossible de recharger l’événement :', error);
    }
  }
  if (!event) { alert('Cet événement n’est plus disponible. Veuillez actualiser la liste des événements.'); return; }
